...
import { Alert, Autocomplete, Box, Button, Card, CardContent, Divider, FormControl, IconButton, InputLabel, MenuItem, Select, Stack, TextField, Typography, Chip } from "@mui/material";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { type FormEvent, useEffect, useState } from "react";

import { testPlexConnection, updateSettings } from "../../api";
import type { ApplicationSettingField, ApplicationSettings, ApplicationSettingsUpdate, PlexConnectionRequest, PlexConnectionResult, SourcePathMapping } from "../../types";

const x264Presets = ["ultrafast", "superfast", "veryfast", "faster", "fast", "medium", "slow", "slower", "veryslow"];
const PLEX_TOKEN_MASK = "●●●●●●●●";
function tokenDraft(settings: ApplicationSettings): string { return settings.plex_token_configured ? PLEX_TOKEN_MASK : ""; }
function enteredToken(value: string): string { const token = value.trim(); return token === PLEX_TOKEN_MASK ? "" : token; }
